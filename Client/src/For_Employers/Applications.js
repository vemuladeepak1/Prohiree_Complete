                ...
                <div class="box">
                    {
                        applications.map((application)=>{
                            ...
                            }
                            {
                              ...
                              (<>
                              <button onClick={()=>updateStatus("shortlisted",`${application._id}`)}>shortlist</button>
                              <button onClick={()=>updateStatus("rejected",`${application._id}`)}>Reject</button>
                              </>)
                              :application.status==="shortlisted"?
                              (<>
                              <button onClick={()=>updateStatus("accepted",`${application._id}`)}>accept</button>
                              <button onClick={()=>updateStatus("rejected",`${application._id}`)}>Reject</button>
                              </>):application.status==="rejected"?
                              <button >Reject</button>:
                              application.status==="accepted"?
                              <button >Accepted</button>:null
                            }
